        ...
        <div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
            ...
            <div class="flex gap-4">
                <div class="flex w-[120px] h-[156px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                    <img src="{{ asset('storage/' . ($transaction->room?->images?->first()->image ?? 'default.jpg')) }}"
                        class="w-full h-full object-cover" alt="icon">
                </div>
                <div class="flex flex-col gap-3 w-full">
                    <p class="font-semibold text-lg leading-[27px]">{{ $transaction->room->name ?? 'kamar tidak ada' }}</p>
                    <hr class="border-[#F1F2F6]">
                    <div class="flex items-center gap-[6px]">
                        <img src="{{ asset('assets/images/icons/profile-2user.svg') }}" class="w-5 h-5 flex shrink-0"
                            alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $transaction->room->capacity ?? 'capacity tidak ada' }}People
                        </p>
                    </div>
                    <div class="flex items-center gap-[6px]">
                        <img src="{{ asset('assets/images/icons/3dcube.svg') }}" class="w-5 h-5 flex shrink-0"
                            alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $transaction->room->square_feet ?? 'td ada' }} sqft flat</p>
                    </div>
                    <hr class="border-[#F1F2F6]">
                    <p class="font-semibold text-lg text-ngekos-orange">
                        {{ number_format($transaction->room->price_per_month ?? 0, 0, ',', '.') }}<span
    ...
    <div
        ...
        <div class="flex flex-col gap-4 pt-[22px]">
            ...
            <div class="flex items-center justify-between">
                ...
                <p class="font-semibold">{{ $transaction->room->price_per_month }}</p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="assets/images/icons/receipt-2.svg" class="w-6 h-6 flex shrink-0" alt="icon">
            ...
            <div class="flex items-center justify-between">
                ...
                @else
                    <p class="font-semibold">{{ $downpayment }}</p>
                @endif
            </div>
